...
#-------- not used ------------#
    ...
    def compute_ngram_to_num(self):
        ...
        for k in self.bg2f:
            ...
            idx += 1
            if 0 == idx%10000: 
                sys.stdout.write('%d/%d processed.\r' % (idx, len(self.bg2f)))
                sys.stdout.flush()
        print
        self.ug2n_once = trans_set_to_num(ug2ws_once)
        self.ug2n_twice = trans_set_to_num(ug2ws_twice)
        ...
        for k in self.tg2f:
            ...
            bg_key = (widl, widm)
            if freq == 1:
                upd_once_bg2n(bg_key, widr, bg2ws_once)
            elif freq == 2:
                upd_once_bg2n(bg_key, widr, bg2ws_twice)
            else:
                upd_once_bg2n(bg_key, widr, bg2ws_other)
            idx += 1
            if 0 == idx%10000:
                sys.stdout.write('%d/%d processed.\r' % (idx, len(self.tg2f)))
                sys.stdout.flush()
        print
        self.bg2n_once = trans_set_to_num(bg2ws_once)
        self.bg2n_twice = trans_set_to_num(bg2ws_twice)
